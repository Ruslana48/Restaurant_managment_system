...
public class CartDaoSql implements CartDao {
    ...
    @Override
    public List<Dish> selectDishesByClient(Client client) {
        ...
        try (Connection con = DriverManager.getConnection(connectionUrl, "root", "xhz2woVLxhz2woVL");) {
            String query = "select dish_id,count from cart_has_dish3 where client_id=?";
    ...
    public List<Dish> dishesFromClientCart(int clientId) throws DbExceptions {
        ...
        try (Connection con = ConnectionPool.getInstance().getConnection();) {
            ...
            ps.setInt(1, clientId);
            System.out.println("ps from method");
    ...
    public int makeAnOrder(int clientId, List<Dish> cart) throws DbExceptions {
        ...
        try (Connection con = ConnectionPool.getInstance().getConnection();) {
            ...
            for (Dish d : cart) {
                ...
                System.out.println("addReceiptHasDish");
            }
        } catch (SQLException e) {
            printSQLException(e);
        }
    ...
    public void setTotalOrderPrice(int user_id, int total, int orderId) {
        String connectionUrl = "jdbc:mysql://localhost:3306/restaurant?serverTimezone=UTC";
        String UPDATE_TOTAL_PRICE_IN_RECEIPT = "UPDATE receipt SET total = ? where user_id=? and id=?";
        try (Connection con = DriverManager.getConnection(connectionUrl, "root", "xhz2woVLxhz2woVL");) {
            PreparedStatement ps = con.prepareStatement(UPDATE_TOTAL_PRICE_IN_RECEIPT);
            System.out.println("ps");
            int k = 0;
            ps.setInt(++k, total);
            ps.setInt(++k, user_id);
    ...
    private void addReceiptHasDish(int orderId, Dish dish, int count) throws SQLException {
        ...
        try (Connection con = DriverManager.getConnection(connectionUrl, "root", "xhz2woVLxhz2woVL");) {
            ...
            System.out.println("ps from add");
            int k = 0;
            ps.setInt(++k, orderId);
            System.out.println(orderId);
    ...
    @Override
    public void updateCount(int quantity, Client client) {
        String UPDATE_COUNT_IN_THE_CART = "UPDATE cart_has_dish3 SET count = ? where client_id=?";
        String connectionUrl = "jdbc:mysql://localhost:3306/restaurant?serverTimezone=UTC";
        try (Connection con = DriverManager.getConnection(connectionUrl, "root", "xhz2woVLxhz2woVL");) {
            PreparedStatement ps = con.prepareStatement(UPDATE_COUNT_IN_THE_CART);
            int k = 0;
            ps.setInt(++k, quantity);
            ps.setInt(++k, client.getId());
            ps.executeUpdate();
    ...
    public int getDishCountInCart(int clientId, String dishId) {
        int dishCount = 0;
        String GET_COUNT_DISH_IN_THE_CART = "select count from cart_has_dish3 where client_id=? and dish_id=?";
        String connectionUrl = "jdbc:mysql://localhost:3306/restaurant?serverTimezone=UTC";
        try (Connection con = DriverManager.getConnection(connectionUrl, "root", "xhz2woVLxhz2woVL");) {
            PreparedStatement ps = con.prepareStatement(GET_COUNT_DISH_IN_THE_CART);
            System.out.println("ps from count");
            int k = 0;
            ps.setInt(++k, clientId);
            ps.setString(++k, dishId);
            ResultSet rs = ps.executeQuery();
